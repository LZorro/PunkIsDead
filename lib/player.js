...
function addPlayer() {
  gbox.addObject({
    ...
    if (this.starsTotal > 0)
	  ...
	  // calculating timer
		...
		if (this.timer > 0)
		{
			...
			//console.log(this.currentNote + ":" + this.timer + ":" + this.freshNoteState);
			...
			{	
				...
				console.log(this.currentNote + ' HIT NOTE NOW ' + level_data[this.currentNote][1]);
			}
			else
			{
				this.goodNoteState = false;
				if (this.freshNoteState)// != this.freshNoteState)// && this.freshNoteState)
				{
					this.currentNote++;
					this.freshNoteState = false;
				}
			}
			//this.freshNoteState = this.goodNoteState;
		}
    },
    ...
    resetHud: function() {
      ...
      for (var y = 0; y < 30; y++)
        for (var x = 0; x < 40; x++)
          if (the_game.level[y][x] == '2') this.starsTotal++;
