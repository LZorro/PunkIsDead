battle_manager = {
  ...
  first: function() {
    ...
    {
      this.timer--;
      var time_index = this.song_data[this.currentNote][0];

      // console.log(this.timer + ' / ' + this.currentNote + ' : ' + this.song_data[this.currentNote]);
      time_index = this.songLength - (time_index * gbox.getFps());

      this.launch_index = this.songLength - (this.song_data[this.currentNote][0] - 2.0) * gbox.getFps();
      ...
      if (this.timer < time_index && this.timer > (time_index-(this.diff_threshold*gbox.getFps())))
      {
        ...
        if (this.song_data[this.currentNote][1] == "attack")
        {
        ...
        else if (this.song_data[this.currentNote][1] == "defend")
        {
        ...
        else
        {
          ...
          //console.log(this.currentNote + ' Press NOW: ' + this.song_data[this.currentNote][1]);
        }
      }
      ...
      if (this.messageState)
      {
        ...
        {
          if (this.currentNote < this.song_data.length-2)
          {
            this.currentNote++;
          }
  ...
  start: function() {
    ...
    console.log('Song length', gbox.getAudioDuration('bgmix'));
    console.log('Song frames', this.songLength);
  ...
  input: function(pressedKey) {
    ...
    //if (gbox.keyIsHit("b")) {
      ...
      //isMusicOn = !isMusicOn;
      if (this.goodNoteState)// && this.song_data[this.currentNote][1] == "x") // add this part to check for specific keys
      {
    ...
    //if (gbox.keyIsHit("c")) {
      ...
      isMusicOn = !isMusicOn;
      if (this.goodNoteState && this.song_data[this.currentNote][1] == "c")
      {
      console.log('Guitars ON');
      gbox.setAudioUnmute('bggtr');
      }
