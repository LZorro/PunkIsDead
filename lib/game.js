var maingame;
var map, foe, yellow_dot, frameCount;
...
function getTileResources(){
  ...
  var tileResources = [
    ...
    {
      id:      'enemy_dot',    // Set a unique ID for future reference
      image:   'enemy_dot',    // Use the 'player_sprite' image, as loaded above
      tileh:   32,
      tilew:   32,
      tilerow: 2,
      gapx:    0,
...
function getAudioResources(){
  ...
  var audioResources = [
	...
	['bgmix', ['sounds/Battletest mix.mp3', 'sounds/Battletest mix.ogg'], { channel: 'bgmix', loop: true }],
	['bggtr', ['sounds/Battletest gtrs.mp3', 'sounds/Battletest gtrs.ogg'], { channel: 'bggtr', loop:true }]
    ];
  if($config.use_plugins){
//    audioResources = [];
    for(var plugin in pluginHelper.loadedPlugins){
